...
export default function Logo({
  ...
  return (
    ...
    >
      <span className="relative flex h-10 w-10 sm:h-11 sm:w-11 shrink-0 items-center justify-center rounded-full overflow-hidden shadow-sm transition-transform duration-300 group-hover:scale-105">
        <Image
          ...
          height={48}
          priority
          className="h-full w-full rounded-full object-cover scale-[1.22] transform"
        />
      </span>
      {showText && (
        <span className="flex flex-col justify-center leading-none">
          ...
          <span className={`text-[10px] sm:text-[11px] tracking-wider uppercase mt-1 ${subColor}`}>
            &amp; HOME TUITION
          </span>
        </span>
      )}
